        ...
        F: Fn(f64) -> f64,
    {
        while let Some(index) = self.equation.iter().rposition(|x| x == &operation) {
            ...
            if let Operation::Number(a) = operand {
                self.equation
                    .splice((index)..=(index + 1), vec![Operation::Number(func(*a))]);
            } else {
        ...
        F: Fn(f64, f64) -> f64,
    {
        while let Some(index) = self.equation.iter().rposition(|x| x == &operation) {
            ...
            if let (Operation::Number(a), Operation::Number(b)) = (firstoperand, secondoperand) {
                self.equation.splice(
                    (index - 1)..=(index + 1),
                    vec![Operation::Number(func(*a, *b))],
                );
            } else {
                bail!("something when wrong calculating")
            }
...
pub fn evaluate(mut equation: Vec<Operation>) -> Result<f64> {
    ...
    while equation.contains(&Operation::OpenParenthesis) {
        while equation
            .get(index)
            .context("ya parenthesis is wrong mate")?
        ...
        let evalparen =
            evaluate_sequence(equation[(open_parenthesis + 1)..=(closed_parenthesis - 1)].to_vec());
        equation.splice(
            open_parenthesis..=closed_parenthesis,
            vec![evalparen.unwrap()],
        );
        index = 0;
    }
